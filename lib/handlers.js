...
// 读取JSON数据文件（支持在根目录或server目录运行）
const readJsonDataFile = (relativePathFromRoot) => {
  let dataPath;
  const currentDir = process.cwd();

  if (currentDir.endsWith('server') || currentDir.includes('server')) {
    dataPath = path.join(currentDir, '..', relativePathFromRoot);
  } else {
    dataPath = path.join(currentDir, relativePathFromRoot);
  }

  if (!fs.existsSync(dataPath)) {
    const altPath = path.join(__dirname, '..', relativePathFromRoot);
    if (fs.existsSync(altPath)) {
      dataPath = altPath;
    } else {
      throw new Error(`找不到数据文件: ${relativePathFromRoot}`);
    }
  }
  ...
  try {
    ...
    const images = result.objects
      ...
      .map(file => {
        ...
        const filename = file.name.replace(basePrefix, '').split('.')[1];
        const catalogItem = catalog.find(item => item.filename === filename);
        const resObj = {
          id: catalogItem?.id,
          index: file.name.replace(basePrefix, '').split('.')[0],
          filename,

  ...
  while (hasMore && tries < maxTries) {
    ...
    if (result.objects) {
      const match = result.objects.find(obj => {
        ...
        // 移除文件扩展名
        const nameWithoutExt = noPrefix.replace(/\.(jpg|jpeg|png|gif|mp3|m4a|wav|ogg|mp4|avi|mov|webm|mkv|flv|wmv)$/i, '');
...
// 获取剧视频链接（从阿里云OSS的video文件夹中获取）
const getDramaVideoHandler = async () => {
  try {
    const videoName = '起来建造';
    console.log(`🎬 开始获取剧视频: ${videoName}.mp4`);
    
    // 使用现有的findOssObjectUrlByBasename函数查找视频（函数会自动匹配扩展名）
    const videoUrl = await findOssObjectUrlByBasename('video', videoName);
    
    if (!videoUrl) {
      const notFound = new Error('未找到视频文件');
      notFound.status = 404;
      throw notFound;
    }
    ...
    if (e.status) throw e;
    const err = new Error(`获取视频失败: ${e && e.message ? e.message : '未知错误'}`);
    err.status = 500;
    err.cause = e;
    throw err;
  }
